...
- The `/api/usage` and `/api/checkout` routes are real. When Stripe or the database are not configured, they return 503. **Checkout 503**: `/api/checkout` returns 503 when `STRIPE_PRICE_SCAN` is not set; create a one-time $2 price in Stripe Dashboard (Products → Add price → One time, $2), copy the `price_xxx` ID, and set `STRIPE_PRICE_SCAN=price_xxx` in `.env.local`. When `RESUME_MATCH_FULL_APP=1`, `/api/usage` returns 503 if the database is unavailable (no 200 with zero counts).
